        ...
        for xyxy, _, confidence, class_id, tracker_id in detections:
            ...
            if(self.pointInZone(bbox_anchor) is True):
                ...
                if(self.triggers.__contains__(tracker_id)):
                    ...
                    self.const_triggers.update({tracker_id: self.n}) # save triggers permanently for use after initial cropping after zone leave
                    #crop_detections.append([tracker_id,xyxy])
                # croppojam triggered detections ik pec n frames
            if(tracker_id in self.const_triggers):
                # print(self.const_triggers.get(tracker_id))
                if(self.const_triggers.get(tracker_id) == self.n):
                    crop_detections.append([tracker_id,xyxy])
                    self.const_triggers[tracker_id] = 1
                else:
                    self.const_triggers[tracker_id] += 1
