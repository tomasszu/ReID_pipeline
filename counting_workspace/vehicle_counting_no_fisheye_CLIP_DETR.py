...
def detections_process(model, frame, tracker, processor):
    confidence_threshold = 0.95

    inputs = processor(images=frame, return_tensors="pt")
    outputs1 = model(**inputs)
    image_size = [frame.shape[0], frame.shape[1]]  # Extract the height and width dimensions
    target_sizes = torch.tensor([image_size])
    results = processor.post_process_object_detection(outputs1, target_sizes=target_sizes, threshold=0.9)[0]
    results["scores"] = results["scores"].clone().detach()
    results["labels"] = results["labels"].clone().detach()
    results["boxes"] = results["boxes"].clone().detach()
...
def frame_annotations(detections, frame, no_labels=False):
...
CLASS_NAMES_DICT = { 1 : "Person", 2 : "Bicycle", 3 : "Car", 4 : "Motorcycle", 5 : "Airplane", 6 : "Bus", 7 : "Train", 8 : "Truck"}
...
tracker = sv.ByteTrack(track_thresh = 0.25, track_buffer = 30, match_thresh = 0.8, frame_rate = 4)
...
#AICITY(4)--------------------------------------------
ZONE1 = counting.countZone(280, 415, 738, -160)
#------------ INTERSECTION 2 ------------------------------------------------------------
...
tracker2 = sv.ByteTrack(track_thresh = 0.25, track_buffer = 30, match_thresh = 0.8, frame_rate = 4)#BYTETrackerArgs())
...
for i in range(int(video.get(cv2.CAP_PROP_FRAME_COUNT))):
    ...
    if(i%2 == 0):
        ...
        detections = detections_process(model, frame, tracker, processor)
...
#         # # -------------------- INTERSECTION 2 -------------------------
        ...
        detections2 = detections_process(model, frame2, tracker2, processor)
...
#         # # -------------------------------------------------------------------
        ...
        annotated_frame2 = frame_annotations(detections2, frame2, no_labels=True)
