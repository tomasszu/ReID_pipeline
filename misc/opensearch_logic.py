...
class Opensearch_db:
    ...
    def insert(self, id, dict):
        ...
        # --- Sanitize vector ---
        if dict['feature_vector'] is None:
            print(f"[WARN] Skipping insert for vehicle {dict['vehicle_id']}: feature_vector is None\n")
            return        

        
        document = {
            "split": dict['split'],
            "camera_id": dict['cam_id'],
            "frame_id": int(id),
            "feature_vector": dict['feature_vector'],
            "bbox": dict['bbox'],
            "vehicle_id": dict['vehicle_id']
        }

        try:
            response = self.client.index(index=self.index_name, body=document, refresh=True)
            print(f"Inserted document, vehicle id {dict['vehicle_id']}: {response}\n")
        except Exception as e:
            print(f"Error inserting document {dict['vehicle_id']}: {e}\n")
