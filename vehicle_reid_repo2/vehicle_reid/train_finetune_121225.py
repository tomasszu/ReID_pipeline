...
This code is meant to finetune the vehicle re-id model on a camera aware dataset.
The goal being to edit the miner/feeder function to only serve positives from different cameras.

Training on the VeRi dataset.
...
parser.add_argument("--checkpoint", default="/home/tomass/tomass/ReID_pipele/vehicle_reid_repo2/vehicle_reid/model/veri+vehixlex_editTrainPar1/net_39.pth")
...
def train_model(model, criterion, start_epoch=0, num_epochs=25, num_workers=2):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for data in loader:
                inputs, labels, cam_ids = data
                now_batch_size = inputs.shape[0]

                if use_gpu or (use_tpu and opt.tpu_cores == 1):
                    inputs, labels, cam_ids = inputs.to(device), labels.to(device), cam_ids.to(device)
