...
from model import ft_net, ft_net_dense, ft_net_hr, ft_net_swin, ft_net_efficient, ft_net_NAS, PCB, MLP
...
def create_model_custom(n_classes, kind="linear", input_dim=26446):
    """Creates a model of a given kind and number of classes"""
    if kind == "linear":
...
def load_model_custom(n_classes, kind="resnet", ckpt=None, remove_classifier=False, **kwargs):
    """Loads a model of a given type and number of classes.
...
def load_model_from_opts_custom_model(opts_file, ckpt=None, return_feature=False, remove_classifier=False, input_dim=26446):
    """Loads a saved model by reading its opts.yaml file.
    ...
    if model_type == "linear":
        model = create_model_custom(n_classes, "linear", input_dim)
    # if model_type in ("resnet", "resnet_ibn"):
    #     model = create_model(n_classes, "resnet", droprate=droprate, ibn=(model_type == "resnet_ibn"),
    #                          stride=stride, circle=return_feature, linear_num=linear_num,
    #                          model_subtype=model_subtype, mixstyle=mixstyle)
