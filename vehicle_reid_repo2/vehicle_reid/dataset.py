...
#<<<<<<<<<<<<<<<<<<<<<<< #### ALTERNATIVE DATASET CLASS IMPLEMENTATION RETURNING ALSO CAMERA ID'S #### >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

class ImageDatasetWCam(Dataset):
    """A labeled image dataset whose annotation is provided as a DataFrame."""
    #also returns cam_id along with image and label

    def __init__(self, img_root, df, target_label, cam_label="cam", classes="infer", transform=None, target_transform=None):
        self.img_root = img_root
        self.df = df
        self.target_label = target_label
