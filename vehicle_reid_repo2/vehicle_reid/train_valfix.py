...
from dataset import ImageDataset, ImageDatasetWCam, BatchSampler
...
parser.add_argument('--data_dir', default='/home/tomass/tomass/data/VeRi', type=str, help='path to the dataset root directory')
#parser.add_argument('--data_dir', default='/home/toms.zinars/tomass/data', type=str, help='path to the dataset root directory')

parser.add_argument("--train_csv_path", default='/home/tomass/tomass/data/VeRi/train.csv', type=str)
#parser.add_argument("--train_csv_path", default='/home/toms.zinars/tomass/data/VeRi_VehicX/VeRi+VehicX_train.csv', type=str)

parser.add_argument("--val_csv_path", default='/home/tomass/tomass/data/VeRi/val.csv', type=str)
...
parser.add_argument('--total_epoch', default=55,
...
parser.add_argument("--checkpoint", default="/home/tomass/tomass/ReID_pipele/vehicle_reid_repo2/vehicle_reid/model/veri_unmodified/net_49.pth")
...
def train_model(model, criterion, start_epoch=0, num_epochs=25, num_workers=2):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        #         dataloaders['train'], [device]).per_device_loader(device)
        # else:
        ...
        with open("vehicle_reid_repo2/vehicle_reid/automated_training/"+ opt.name +".txt", "a") as file:
            # Write some lines to the file
            file.write('{},{},{:.4f},{:.4f}\n'.format(
            epoch, 'val', metrics['rank1'], metrics['mAP'])) #, metrics['roc_auc']))
...
def compute_rank_map_wCam(feats, ids, cams):
    ''' Compute Rank-1 and mAP with camera IDs taken into account '''
    dist = cdist(feats, feats, metric="cosine")

    print("--------------------------------------------------------------")
    print("Rank1 w Cams sanity check: ", "Unique cams:", np.unique(cams))
    print("IDs with >=2 cams:",
      np.sum([len(np.unique(cams[ids == i])) > 1 for i in np.unique(ids)]))
    print("--------------------------------------------------------------")
    ...
    for i in range(len(ids)):
        ...
        valid = ~((ids[order] == ids[i]) & (cams[order] == cams[i]))
        order = order[valid]
    ...
    rank1 = rank1 / valid_q if valid_q > 0 else 0.0
